...
use crate::model::{ParamKind, ParamSpec, text, vector};
...
/// The maps offered, in [`ColorMap::named`] spelling.
///
...
pub const MAPS: &[&str] = &["viridis", "cool-warm", "grayscale"];
...
/// `volume` and [`contour`](super::contour) do —
...
fn run(request: &Request) -> Outcome {
    ...
    if chosen == "categorical" || chosen == "element" {
        ...
        for value in &scalars {
            ...
            let rgb = match chosen {
                "element" => {
                    let rgba = crate::data::chem::colour(key);
    ...
    let map = ColorMap::named(chosen).unwrap_or_default();
...
pub fn sample(map: ColorMap, t: f32) -> [f32; 4] {
