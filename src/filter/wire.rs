...
use crate::data::array::{BufferMeta, GeometryMeta};
use crate::model::SceneError;
use crate::model::{ParamMap, ParamSpec};
...
impl Graph {
    ...
    fn record_reads(&mut self, registry: &FilterRegistry, id: u64, kind: &str, params: &ParamMap) {
        ...
        let reads = registered
            .inputs()
            .filter_map(|spec| crate::model::data(params, spec.id))
            .collect();
...
pub fn set(
    ...
    for (key, value) in params {
        ...
        match value {
            crate::model::ParamValue::Unset => wanted.remove(&key),
    ...
    for spec in registered.inputs() {
        let Some(handle) = crate::model::data(&wanted, spec.id) else {
            continue;
        };
...
/// The binding gate for a filter kind. See [`crate::model::check_bindings`],
