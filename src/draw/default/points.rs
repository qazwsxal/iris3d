...
///
/// Built from `CARGO_CRATE_NAME` rather than written out, because that is the
/// prefix `embedded_asset!` registers under and a hand-written one goes stale
/// silently the moment the crate is renamed — the material still loads, the
/// asset server logs a missing path, and points are the only thing that stops
/// drawing. `Material::vertex_shader` is a static method with no asset server
/// in reach, so `load_embedded_asset!` is not available here the way it is in
/// `pipeline`.
const SHADER: &str = concat!(
    "embedded://",
    env!("CARGO_CRATE_NAME"),
    "/draw/default/point_quad.wgsl"
);
...
mod tests {
    use super::*;
    use crate::data::array::{BufferMeta, Dtype};
    ...
    fn app() -> (App, Entity, Entity) {
        ...
        app.init_resource::<crate::scene::registry::ActorRegistry>();
        register(
            &mut app
                .world_mut()
                .resource_mut::<crate::scene::registry::ActorRegistry>(),
        );
        app.add_systems(
            Update,
            (
