...
// Per sample under MSAA, and that is the point of doing it here rather than in
// the accumulation. Each sample of the view target holds its own background
// colour, so multiplying them all by one transmittance would throw the
// volume's antialiased edge away again — the coverage the accumulation
// captured would be resolved back into a hard edge. Reading
// `@builtin(sample_index)` is what makes this shader run per sample; it costs
// one cheap fullscreen pass times the sample count, against the whole
// second geometry pass if the accumulation did the same.
@fragment
fn fragment(
...
#ifdef MULTISAMPLED
    let totals = textureLoad(totals_texture, coord, i32(sample_index));
#else
