//! The experimental backend: moment-based order-independent transparency.
//!
//! Where [`super::default`] draws a mesh as a lit shell, this pathway treats a
//! closed mesh as the boundary of a solid that absorbs light at a uniform rate.
//! What you see is the interior: thick parts read dark, thin parts read clear,
//! and nested or overlapping volumes compose correctly whatever order they are
//! drawn in.
//!
...
//! # What it draws
//!
//! One kind so far: `surface`, which is `shared: true` because a closed
//! triangle mesh is the same physical thing whichever pathway draws it. There
//! is no `volume` yet — a sampled grid has no closed boundary to take the
//! signed prefix of, so it wants froxels rather than a missing line of code,
//! and an unregistered kind is refused by name rather than drawn wrongly.
//!
...
///
/// Split out from [`ExperimentalBackendPlugin`] so it can be added without the
...
pub struct ExperimentalBackendPlugin;

impl Plugin for ExperimentalBackendPlugin {
    fn build(&self, app: &mut App) {
...
/// [`pipeline`]. Only the solari pathway has to force `Msaa::Off`, and it does
