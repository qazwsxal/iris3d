...
//!
//! The same `surface` id the other two backends register, and `shared: true`
//! for the reason that word exists: a triangle mesh is one physical thing
//! whether it is rasterised, raytraced, or — here — integrated through. What
//! differs is the picture, not the object, which is exactly the case the shared
//! vocabulary is for.
//!
//! The parameters differ, and that is expected: a client asks `ListActorKinds`
//! and reads them rather than assuming. `double_sided` is gone, because it was
//! a lighting choice and both faces are always drawn here — they *have* to be,
//! since the two of them are the endpoints of the interval being integrated.
//!
...
pub fn register(registry: &mut ActorRegistry) {
    registry.register(ActorKind {
        ...
        // A closed triangle mesh is one mesh, integrated through or lit.
        shared: true,
        params: PARAMS,
        apply: |entity, params| {
...
fn normal_reflectance(ior: f32) -> f32 {
...
fn transmission(colour: &ColorBy) -> Vec3 {
