...
//!   that raises it.
//!
//! The arrays themselves are in [`iris3d_data`], flat and by handle: they belong
...
/// The bus carrying [`SceneCommand`]s. See [`Bus`](iris3d_core::bus::Bus).
pub type CommandBus = iris3d_core::bus::Bus<SceneCommand>;

pub use iris3d_data::{BufferMeta, DataArray, DataStore, Dtype, HeldMeta, NamedBuffer};
/// Re-exported: the scene raises most of these, and a caller that thinks of a
/// refusal as a scene fact can keep naming it one. The type itself lives in
/// [`iris3d_model`], because the filter graph raises them too.
pub use iris3d_model::SceneError;
...
pub struct ActorSummary {
    ...
    pub parents: Vec<u64>,
    pub params: iris3d_model::ParamMap,
    pub visible: bool,
}
...
pub struct KindSummary {
    pub id: String,
    pub label: String,
    pub params: &'static [iris3d_model::ParamSpec],
}
...
fn set_actor(
    ...
    for (key, value) in params {
        ...
        match value {
            iris3d_model::ParamValue::Unset => merged.remove(&key),
...
mod tests {
    ...
    fn add(app: &mut App, parents: Vec<u64>) -> u64 {
        let mut added = send(app, |reply| SceneCommand::AddActor {
            parents,
            kind: "marker".into(),
            params: iris3d_model::ParamMap::default(),
            reply,
        });
        app.update();
    ...
    fn an_actor_keeps_its_binding_when_a_rebind_is_refused() {
        const INPUTS: &[iris3d_model::ParamSpec] = &[iris3d_model::ParamSpec {
            id: "field",
            label: "Field",
        ...
        let bind = |handle: u64| {
            ...
            params.insert("field".into(), iris3d_model::ParamValue::Data(handle));
            params
        };
        ...
        let bound = |app: &mut App| {
            ...
            iris3d_model::data(&listing[0].params, "field")
        };
        assert_eq!(
            bound(&mut app),
            Some(first),
    ...
    fn an_optional_input_can_be_unbound_and_a_required_one_cannot() {
        const INPUTS: &[iris3d_model::ParamSpec] = &[
        ...
        let mut params = iris3d_model::ParamMap::default();
        params.insert("field".into(), iris3d_model::ParamValue::Data(handles[0]));
        params.insert("mask".into(), iris3d_model::ParamValue::Data(handles[1]));
        let mut added = send(&app, |reply| SceneCommand::AddActor {
            parents: vec![],
            kind: "maskable".into(),
            params,
        ...
        let clear = |input: &str| {
            let mut params = iris3d_model::ParamMap::default();
            params.insert(input.into(), iris3d_model::ParamValue::Unset);
            params
        };
        let bound = |app: &mut App, input: &'static str| {
            ...
            app.update();
            let listing = listed.try_recv().expect("a reply").expect("a listing");
            iris3d_model::data(&listing[0].params, input)
        };
    ...
    fn listing_kinds_reports_every_registered_kind() {
        const PARAMS: &[iris3d_model::ParamSpec] = &[iris3d_model::ParamSpec {
            id: "size",
            label: "size",
            kind: iris3d_model::ParamKind::Float {
                default: 1.0,
                min: 0.0,
                max: 2.0,
                logarithmic: false,
