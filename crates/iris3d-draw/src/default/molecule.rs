...
//!
//! The tessellation is [`crate::atoms`]. What is here is where it sits:
...
use crate::atoms::{self, Layout, Sizes};
use iris3d_model::{Bindings, ParamKind, ParamSpec, float};
    ...
    let colours: Vec<[f32; 4]> = (0..positions.len())
        .map(|index| {
            bound_rgb.as_ref().map_or_else(
                || iris3d_data::chem::colour(elements.get(index).copied().unwrap_or(6)),
