...
//!
//! Absorbance accumulates up to the **opaque depth**, so a density map in front
//! of a ribbon dims it and one behind does not, which is what lets a structure
//! be shown inside the map it was built from.
//!
//! Absorbance is additive and addition does not care about order, so it is
//! accumulated with an additive blend and transmittance `T = exp(-A)` is
//! reconstructed in a second pass. A fragment contributes using only its own
//! depth: front faces add `-F_k(w)` and back faces `+F_k(w)`, and the blend
//! performs the pairing on its own. **`docs/design/moment-transparency.md` has
//! the derivation, what four moments buy, and what is still unbuilt.**
//!
//! # What it draws
//!
...
//!
//! A ribbon is generated by `filter::cartoon` and
//! bound to either of those, or to both at once, rather than being a kind that
//! generates and draws in one place.
//!
...
impl Plugin for MomentRenderPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins(ExtractComponentPlugin::<MomentView>::default())
            ...
            // depth buffer.
            .add_systems(Update, (moment_cameras, widen_depth_usage).chain());
    }
