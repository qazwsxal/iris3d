...
//! `volume` reads one per ray sample for the
...
use iris3d_model::{ParamKind, ParamSpec, float, text, uvec3, vec3, vector};
...
fn colours(request: &Request, grid: UVec3, step: UVec3, at: &[Vec3]) -> Option<Vec<[f32; 4]>> {
    ...
    let map = ColorMap::named(text(&request.params, "map", "viridis")).unwrap_or_default();
    let width = high - low;
    Some(
        sampled
            .iter()
...
mod tests {
    ...
    use iris3d_scene::{DataArray, Dtype};
