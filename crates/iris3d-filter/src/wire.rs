...
//!
//! The actor half of this lives in [`scene`](iris3d_scene) beside the commands
...
/// [`ActorQuery`](iris3d_scene) is one query: a read-only query over the same
...
impl Graph {
    ...
    fn record_reads(&mut self, registry: &FilterRegistry, id: u64, kind: &str, params: &ParamMap) {
        ...
        let reads = registered
            .inputs()
            .filter_map(|spec| iris3d_model::data(params, spec.id))
            .collect();
...
pub fn set(
    ...
    for (key, value) in params {
        ...
        match value {
            iris3d_model::ParamValue::Unset => wanted.remove(&key),
    ...
    for spec in registered.inputs() {
        let Some(handle) = iris3d_model::data(&wanted, spec.id) else {
            continue;
        };
...
///
/// Separate from [`SceneCommand`](iris3d_scene::SceneCommand) because filters
