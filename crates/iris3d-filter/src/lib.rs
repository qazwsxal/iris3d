...
//! and lives outside `draw` entirely.
//!
...
//! `draw::mark_dirty` watches — so consumers re-dirty with no
...
use iris3d_model::{Bindings, ParamMap, ParamSpec};
...
///
/// The mirror of [`ParamKind`](iris3d_model::ParamKind) on the way in,
...
pub enum OutputKind {
    ...
    /// [`GeometryMeta`](iris3d_data::array::GeometryMeta) afterwards rather than
...
/// [`ActorKind`](iris3d_scene::registry::ActorKind): same `ParamSpec`
...
///
/// Unlike [`ActorRegistry`](iris3d_scene::registry::ActorRegistry) this is not
...
/// `ActorParams`(iris3d_scene::registry::ActorParams) is for an actor.
...
///
/// Not graded the way `Dirty` is. An actor can repaint
...
/// work after it.
///
/// Before `Invalidate` rather than inside it: a filter finishing
...
impl Plugin for FilterPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.configure_sets(
            Update,
            Filter.after(iris3d_scene::registry::apply_actor_params),
        )
        .add_systems(
            Update,
        ...
        .add_systems(
            ...
            wire::apply_filter_commands
                .after(iris3d_scene::apply_scene_commands)
                .before(Filter),
        );
...
pub(crate) fn describe(name: &str, mesh: &Mesh) -> iris3d_data::array::GeometryMeta {
